...
//Intercalação Interna utilizando 
...
// Função de intercalação balanceada de blocos
...
{   
    ...
    {
        // Zera a contagem de leituras para cada fita atual
        for (i = 0; i < quant_fitas_escritas; i++)
            quant_leituras_atual_fita[i] = 0;

        // Lê um registro de cada fita de entrada
        for (i = 0; i < quant_fitas_escritas; i++)
        {
        ...
        // Realiza a intercalação dos registros
        while (tam_novo_bloco > nro_atual_escritas && nro_registros > nro_atual_escritas)
        {
            // Encontra a posição do menor registro no bloco de memória interna
            ...
            // Escreve o menor registro no destino
            ...
            // Lê o próximo registro da fita de origem, se necessário
            if (!fita_completa[pos_menor])
            {
                ...
                // Marca a fita como completa, se necessário
                if (quant_leituras_atual_fita[pos_menor] == tam_antigo_bloco || quant_leituras_atual_fita[pos_menor] == origem[pos_menor].quant_itens)
                {
            ...
            {
                // Marca a fita como completa e define o valor do registro como infinito, se necessário
        ...
        // Atualiza as informações da fita de destino e fitas de origem
        ...
        // Reinicia contadores e informações para a próxima iteração
...
// Função auxiliar para encontrar a posição do menor registro no vetor
